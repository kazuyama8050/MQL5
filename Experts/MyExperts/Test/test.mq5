...
//       // request.magic    =MAGIC_NUMBER_TEST;             // ポジションのMagicNumber
//       request.type_filling=ORDER_FILLING_IOC;   //フィルポリシー
      
//       //--- ポジションタイプによる注文タイプと価格の設定
//       //    ポジションタイプの逆のものを指定する（買いポジションなら売りオペ）
...
//       if(!OrderSend(request,result)) {
//           PrintFormat("ALL Position Down OrderSend error %d",GetLastError());     // リクエストの送信が失敗した場合、エラーコードを出力する
//         //--- 操作に関する情報
//         PrintFormat("retcode=%u  deal=%I64u  order=%I64u",result.retcode,result.deal,result.order);
//       }

//       if (result.retcode == 200) {
//         PrintFormat("Success Position Down : symbol = %s, volume = %d, price = %d, comment = %s", _Symbol, result.volume, result.price, result.comment);
//       } else {
//         PrintFormat("Failed Position Down : symbol = %s, comment = %s", request.symbol, result.comment);
//       }
